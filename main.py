...
IMAGE_PATH = "Goose"
PLAYR_IMAGES = os.listdir(IMAGE_PATH)
...
def create_enemy():
    ...
    enemy_rect = pygame.Rect(WIDHT, random.randint(enemy.get_height(), HEIGHT - enemy.get_height()), *enemy_size)
...
def create_bonus():
    ...
    bonus_rect = pygame.Rect(random.randint(bonus_width, WIDHT - bonus_width), -bonus.get_height(), *bonus_size)
...
while playing:  
    ...
    for event in pygame.event.get ():
        ...
        if event.type == CHANGE_IMAGE:
            player = pygame.image.load(os.path.join(IMAGE_PATH, PLAYR_IMAGES[image_index]))
            image_index += 1
            if image_index >= len(PLAYR_IMAGES):
                image_index = 0
